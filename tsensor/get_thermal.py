...
flags.DEFINE_integer(
    "min_temperature", 29815, "the minimum body temperature in centi kelvin"
)
flags.DEFINE_integer(
    "max_temperature", 31815, "the maximum expected body temperature in centi kelvin"
)
flags.DEFINE_string(
    "face_model",
    "thermal_face_automl_edge_fast_edgetpu.tflite",
    "The TF Lite face detection model file compiled for edge TPU",
)
flags.DEFINE_float(
    "face_confidence", 0.5, "the confidence threshold for face detection"
)
flags.DEFINE_integer(
    "max_num_faces", 10, "the maximum supported of face detect per frame"
)
            ...
            try:
                ...
                if FLAGS.detect:
                    if not ambient.get_sensor_data():
                        ...
                        logging.debug(
                            "Ambient temperature : %.f °C" % ambient_data.temperature
                        )
                        logging.debug(
                            "Ambient pressure: %.f hPa" % ambient_data.pressure
                        )
                        logging.debug(
                            "Ambient humidity: %.f %%" % ambient_data.humidity
                        )
                        ...
                        if FLAGS.visualize:
                            ...
                            if FLAGS.detect:
                                for face in faces:
                                    ...
                                    top_left = int(
                                        window_scale_factor_x * bbox[0, 0]
                                    ), int(window_scale_factor_y * bbox[0, 1])
                                    bottom_right = int(
                                        window_scale_factor_x * bbox[1, 0]
